    ...
    while True:
        try:
            ...
            with open(file_path, "r") as csv_file:
                ...
                required_fields = {"temperature", "status"}
                if not required_fields.issubset(csv_reader.fieldnames):
                    logger.error(f"Missing required columns: {required_fields - set(csv_reader.fieldnames)}")
                    sys.exit(1)

                for row in csv_reader:
                    # Ensure required fields are present
                    if "temperature" not in row or "status" not in row:
                        logger.error(f"Missing 'temperature' or 'status' column in row: {row}")
                        continue

                    # Generate timestamp
                    current_timestamp = datetime.utcnow().isoformat()
                    
                    # Process status safely
                    status = str(row["status"]).lower()
                    temperature = float(row["temperature"])
                    
                    # Create a unique message based on status
                    if status == "low":
                        alert_message = "Warning: Temperature is low. Monitor closely."
                    elif status == "high":
                        alert_message = "ALERT: Temperature is too high! Take immediate action!"
                    else:
                        alert_message = "Temperature is stable."
